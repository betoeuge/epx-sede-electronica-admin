...
import { THEMES, TITLE_FONTS, BODY_FONTS, HEADER_STYLES, FOOTER_STYLES, DEFAULT_FOOTER_DATA } from './editorConstants';
...
/** Injects a Google Fonts <link> for the given font name if not already loaded. */
...
function FontSelector({ value, options, onChange }) {
  useEffect(() => { loadGoogleFont(value); }, [value]);
  const handleChange = (v) => { loadGoogleFont(v); onChange(v); };
  return (
    <Selector
      value={value}
      options={options}
      onChange={handleChange}
      icon={<div style={{ width: '1.125rem', height: '1.125rem', border: '0.0625rem solid var(--text-dim)', color: 'var(--text-dim)', borderRadius: '0.125rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.625rem', flexShrink: 0 }}>T</div>}
    />
  );
}

function ThemeSelector({ value, options, onChange }) {
  ...
  return (
    <Selector
      ...
      options={options}
      onChange={onChange}
      icon={value !== 'Costumizado' && selectedTheme && (
         <div style={{ width: '1.125rem', height: '1.125rem', borderRadius: '0.25rem', border: '0.0625rem solid white', background: selectedTheme.primary, flexShrink: 0 }} />
      )}
    />
  );
}
...
export function SiteSettingsPanel({
  ...
  const [currentView, setCurrentView] = useState('main'); // 'main', 'navigation', 'accessibility', 'leftLinks'
  ...
  // ── Accessibility sub-view ──────────────────────────────────────────────────
  if (currentView === 'accessibility') {
    const a11yData = editorState.accessibilityData || {};
    const customItems = a11yData.customItems || [];
    const inputStyle = { background: 'var(--bg-darker)', border: '0.0625rem solid var(--border-subtle)', borderRadius: '0.5rem', padding: '0.5rem 0.875rem', color: 'var(--text-dim)', fontSize: '0.875rem', fontFamily: "'Inter', sans-serif", width: '100%', boxSizing: 'border-box', outline: 'none' };
    const lblStyle = { fontFamily: "'Inter', sans-serif", fontSize: '0.75rem', color: 'var(--text-dim)', fontWeight: 500 };

    const updateCustomItems = (newItems) => onAccessibilityDataChange('customItems', newItems);
    ...
    return (
      <div style={{ width, flexShrink: 0, background: 'var(--bg-main)', borderLeft: '0.0625rem solid var(--border-subtle)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        <div style={{ background: 'var(--bg-darker)', borderBottom: '0.0625rem solid var(--border-subtle)', display: 'flex', alignItems: 'center', padding: '0 1rem', gap: '0.75rem', height: '3.125rem', boxSizing: 'border-box', flexShrink: 0 }}>
          ...
          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.9375rem', fontWeight: 500, color: 'var(--text-active)', flex: 1 }}>Barra de Accesibilidad</span>
        </div>

        <Accordion title="Elementos obligatorios" isOpen={openPanels.a11yFixed ?? true} onToggle={() => togglePanel('a11yFixed')}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {[
              { icon: FaAdjust, label: 'Contraste' },
              { label: 'Aumentar letra', icon: (props) => <MaskIcon icon={aZoomPlus} {...props} /> },
              { label: 'Reducir letra', icon: (props) => <MaskIcon icon={aZoom} {...props} /> },
            ].map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0.625rem', background: 'rgba(255,255,255,0.03)', borderRadius: '0.375rem', border: '0.0625rem solid var(--border-subtle)' }}>
                <div style={{ width: '1.5rem', height: '1.5rem', background: 'rgba(255,255,255,0.1)', borderRadius: '0.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <item.icon size={11} color="white" />
                </div>
                <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.8125rem', color: 'var(--text-active)', flex: 1 }}>{item.label}</span>
                <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="var(--text-dim)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
              </div>
            ))}
            <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.6875rem', color: 'var(--text-dim)', margin: '0.25rem 0 0 0', lineHeight: 1.4 }}>
              Estos elementos son obligatorios y siempre estarán visibles en la barra.
            </p>
          </div>
        </Accordion>

        <Accordion title={`Elementos personalizados (${customItems.length})`} isOpen={openPanels.a11yCustom ?? true} onToggle={() => togglePanel('a11yCustom')}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {customItems.map((item, i) => {
              const iconData = GOV_ICONS[item.icon];
              const IconComp = iconData ? iconData.icon : null;
              const isEditingIcon = editingIconId === item.id;
              return (
                <div key={item.id} style={{ background: 'rgba(255,255,255,0.03)', border: '0.0625rem solid var(--border-subtle)', borderRadius: '0.5rem', padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <button onClick={() => setEditingIconId(isEditingIcon ? null : item.id)} title="Cambiar ícono" style={{ width: '1.75rem', height: '1.75rem', background: 'rgba(255,255,255,0.1)', border: '0.0625rem solid var(--border-subtle)', borderRadius: '0.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', padding: 0 }}>
                        {IconComp ? <IconComp size={13} color="white" /> : <span style={{ color: 'white', fontSize: '0.625rem' }}>?</span>}
                      </button>
                      <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.8125rem', color: 'var(--text-active)', fontWeight: 500 }}>{item.label || 'Sin nombre'}</span>
                    </div>
                    <button onClick={() => { const newItems = customItems.filter((_, idx) => idx !== i); updateCustomItems(newItems); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  {isEditingIcon && (
                    <IconSelector value={item.icon} onChange={newIcon => {
                      const newItems = [...customItems];
                      newItems[i] = { ...item, icon: newIcon };
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                    ...
                    <input value={item.label} onChange={e => { const newItems = [...customItems]; newItems[i] = { ...item, label: e.target.value }; updateCustomItems(newItems); }} style={inputStyle} placeholder="Ej: Centro de Relevo" />
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                    <span style={lblStyle}>Tipo de enlace</span>
                    <select value={item.target || 'external'} onChange={e => { const newItems = [...customItems]; newItems[i] = { ...item, target: e.target.value, url: e.target.value === 'external' ? (item.url || '') : '' }; updateCustomItems(newItems); }} style={{ ...inputStyle, appearance: 'auto' }}>
                      <option value="external">URL Externa</option>
                      {(editorState.pages || []).map(page => (
                        <option key={page.id} value={page.id}>{page.label} (Página interna)</option>
                      ))}
                    </select>
                  </div>
                  {(item.target === 'external' || !item.target) && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                      <span style={lblStyle}>URL</span>
                      <div style={{ display: 'flex', alignItems: 'stretch', border: '0.0625rem solid var(--border-subtle)', borderRadius: '0.5rem', overflow: 'hidden' }}>
                        <span style={{ padding: '0 0.5rem', color: 'var(--text-dim)', fontSize: '0.75rem', borderRight: '0.0625rem solid var(--border-subtle)', height: '100%', display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.03)', flexShrink: 0, userSelect: 'none' }}>
                          <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>
                        </span>
                        <input value={item.url || ''} onChange={e => { const newItems = [...customItems]; newItems[i] = { ...item, url: e.target.value }; updateCustomItems(newItems); }} style={{ ...inputStyle, border: 'none', borderRadius: 0, background: 'transparent', outline: 'none', flex: 1, padding: '0.5rem 0.625rem', fontSize: '0.8125rem' }} placeholder="https://..." />
                      </div>
                    </div>
                  )}
                </div>
            ...
            <button
              onClick={() => { const newItem = { id: 'a11y-' + Date.now(), icon: 'link', label: 'Nuevo enlace', url: '#', target: 'external' }; updateCustomItems([...customItems, newItem]); }}
              style={{ background: 'rgba(255,255,255,0.05)', border: '0.0625rem dashed var(--border-subtle)', borderRadius: '0.5rem', padding: '0.625rem', color: 'var(--text-dim)', cursor: 'pointer', fontFamily: "'Inter', sans-serif", fontSize: '0.8125rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.375rem', transition: 'all 0.2s' }}
  ...
  // ── Left Links sub-view ─────────────────────────────────────────────────────
  if (currentView === 'leftLinks') {
    const llData = editorState.leftLinksData || {};
    const customItems = llData.customItems || [];
    const inputStyle = { background: 'var(--bg-darker)', border: '0.0625rem solid var(--border-subtle)', borderRadius: '0.5rem', padding: '0.5rem 0.875rem', color: 'var(--text-dim)', fontSize: '0.875rem', fontFamily: "'Inter', sans-serif", width: '100%', boxSizing: 'border-box', outline: 'none' };
    const lblStyle = { fontFamily: "'Inter', sans-serif", fontSize: '0.75rem', color: 'var(--text-dim)', fontWeight: 500 };

    const updateCustomItems = (newItems) => onLeftLinksDataChange('customItems', newItems);
    ...
    return (
      <div style={{ width, flexShrink: 0, background: 'var(--bg-main)', borderLeft: '0.0625rem solid var(--border-subtle)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        <div style={{ background: 'var(--bg-darker)', borderBottom: '0.0625rem solid var(--border-subtle)', display: 'flex', alignItems: 'center', padding: '0 1rem', gap: '0.75rem', height: '3.125rem', boxSizing: 'border-box', flexShrink: 0 }}>
          ...
          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.9375rem', fontWeight: 500, color: 'var(--text-active)', flex: 1 }}>Barra Izquierda</span>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '1rem' }}>
          {customItems.map((item, i) => {
            ...
            return (
              <div key={item.id} style={{ background: 'rgba(255,255,255,0.03)', border: '0.0625rem solid var(--border-subtle)', borderRadius: '0.5rem', padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <span style={lblStyle}>Nombre</span>
                  <input value={item.label} onChange={e => { const newItems = [...customItems]; newItems[i] = { ...item, label: e.target.value }; updateCustomItems(newItems); }} style={inputStyle} placeholder="Ej: Nuevo enlace" />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  <span style={lblStyle}>Tipo de enlace</span>
                  <select value={item.target || 'external'} onChange={e => { const newItems = [...customItems]; newItems[i] = { ...item, target: e.target.value, url: e.target.value === 'external' ? (item.url || '') : '' }; updateCustomItems(newItems); }} style={{ ...inputStyle, appearance: 'auto' }}>
          ...
          <button
            onClick={() => { const newItem = { id: 'll-' + Date.now(), icon: 'link', label: 'Nuevo enlace', url: '#', target: 'external' }; updateCustomItems([...customItems, newItem]); }}
            style={{ background: 'rgba(255,255,255,0.05)', border: '0.0625rem dashed var(--border-subtle)', borderRadius: '0.5rem', padding: '0.625rem', color: 'var(--text-dim)', cursor: 'pointer', fontFamily: "'Inter', sans-serif", fontSize: '0.8125rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.375rem', transition: 'all 0.2s' }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.1)'; e.currentTarget.style.color = 'white'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.color = 'var(--text-dim)'; }}
          >
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem'}}>
                  ...
                  <select value={navType} onChange={e => {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem'}}>
                  <span style={lblStyle}>Nombre</span>
                  <input value={nav.label} onChange={e => updateNavItem(nav.id, { label: e.target.value })} style={inputStyle} />
                </div>
                {navType === 'simple' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem'}}>
                    <span style={lblStyle}>Hyper Vinculo</span>
                    <div style={{ position: 'relative' }}>
                      <select value={nav.target || ''} onChange={e => updateNavItem(nav.id, { target: e.target.value })} style={selectStyle}>
                ...
                {navType === 'dropdown' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <button onClick={() => addChildToDropdown(nav.id)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '0.25rem', width: '1.5rem', height: '1.5rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    {(nav.children || []).map(child => (
                      <div key={child.id} style={{ background: 'var(--bg-hover)', borderRadius: '0.375rem', padding: '0.5rem 0.625rem', display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
                          ...
                          <div onClick={() => removeChild(nav.id, child.id)} style={{ padding: '0.25rem', borderRadius: '0.25rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-dim)' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(235,87,87,0.15)'; e.currentTarget.style.color = '#eb5757'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-dim)'; }} title="Eliminar sub-item">
                        ...
                        <input value={child.label} onChange={e => updateChild(nav.id, child.id, { label: e.target.value })} placeholder="Nombre" style={{ ...inputStyle, fontSize: '0.8125rem', padding: '0.375rem 0.625rem' }} />
                        <select value={child.target || ''} onChange={e => updateChild(nav.id, child.id, { target: e.target.value })} style={{ ...selectStyle, fontSize: '0.8125rem', padding: '0.375rem 0.625rem' }}>
                          <option value="">Añadir vinculo</option>
                          {editorState.pages.map(p => (
                            <option key={p.id} value={p.id}>/{p.label}</option>
                          ))}
    ...
    <div style={{ width, flexShrink: 0, background: 'var(--bg-main)', borderLeft: '0.0625rem solid var(--border-subtle)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      ...
      <Accordion title="Accesibilidad" isOpen={openPanels.accessibility} onToggle={() => togglePanel('accessibility')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <button
            ...
            style={{ background: 'rgba(255,255,255,0.05)', border: '0.0625rem solid var(--border-subtle)', borderRadius: '0.5rem', padding: '0.5rem 0.875rem', color: 'var(--text-dim)', cursor: 'pointer', fontFamily: "'Inter', sans-serif", fontSize: '0.8125rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'all 0.2s', width: '100%' }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.1)'; e.currentTarget.style.color = 'white'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.color = 'var(--text-dim)'; }}
          >
            <span>Configurar elementos</span>
            <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </button>
        </div>
      </Accordion>
      ...
      <Accordion title="Barra Izquierda (Enlaces)" isOpen={openPanels.leftLinks} onToggle={() => togglePanel('leftLinks')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <button
            onClick={() => setCurrentView('leftLinks')}
            style={{ background: 'rgba(255,255,255,0.05)', border: '0.0625rem solid var(--border-subtle)', borderRadius: '0.5rem', padding: '0.5rem 0.875rem', color: 'var(--text-dim)', cursor: 'pointer', fontFamily: "'Inter', sans-serif", fontSize: '0.8125rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'all 0.2s', width: '100%' }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.1)'; e.currentTarget.style.color = 'white'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.color = 'var(--text-dim)'; }}
          >
      ...
      <Accordion title="Fuentes del sitio" isOpen={openPanels.fuentes} onToggle={() => togglePanel('fuentes')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.875rem', color: 'var(--text-active)' }}>Titulos</span>
          <FontSelector value={editorState.fontTitles} options={TITLE_FONTS} onChange={onFontTitles} />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.875rem', color: 'var(--text-active)' }}>Textos</span>
          <FontSelector value={editorState.fontBody} options={BODY_FONTS} onChange={onFontBody} />
        </div>
      </Accordion>

      {/* ── Tema del sitio ─────────────────────────────────────────────────── */}
      <Accordion title="Tema del sitio" isOpen={openPanels.tema} onToggle={() => togglePanel('tema')}>
        <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.75rem', color: 'var(--text-active)', margin: 0 }}>
          Selecciona un tema relacionado a la entidad del sitio
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
          <ThemeSelector
            value={editorState.themeName}
      ...
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <Accordion title="Header" isOpen={openPanels.headerType} onToggle={() => togglePanel('headerType')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
      ...
      <Accordion title="Logos del Header" isOpen={openPanels.headerLogos} onToggle={() => togglePanel('headerLogos')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem'}}>
          <LogoUploader
      ...
      <Accordion title="Navegación" isOpen={openPanels.menu} onToggle={() => togglePanel('menu')}>
      ...
      {/* ── Footer ─────────────────────────────────────────────────────────── */}
      <Accordion title="Footer" isOpen={openPanels.footerType} onToggle={() => togglePanel('footerType')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
          <Selector
      ...
      {(() => {
        ...
        return (
          <Accordion title="Secciones Visibles del Footer" isOpen={openPanels.footerVisibility ?? false} onToggle={() => togglePanel('footerVisibility')}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      {(() => {
        ...
        return (
          <Accordion title="Información Principal del Footer" isOpen={openPanels.footerMain ?? false} onToggle={() => togglePanel('footerMain')}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {v2Hint}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
      ...
      {(editorState.footerStyle || 'version01') === 'version01' && (
        <Accordion title={`Sedes (${(fd.sedes || []).length})`} isOpen={openPanels.footerSedes ?? false} onToggle={() => togglePanel('footerSedes')}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      ...
      <Accordion title={`Links del Footer (${(fd.footerLinks || []).length})`} isOpen={openPanels.footerLinks ?? false} onToggle={() => togglePanel('footerLinks')}>
      ...
      <div style={{ padding: '1rem', display: 'flex', justifyContent: 'center' }}>
        ...
        >
          Restaurar valores por defecto del Footer
        </button>
      </div>
    </div>
  );
