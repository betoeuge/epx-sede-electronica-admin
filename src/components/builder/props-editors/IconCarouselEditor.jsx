...
export function IconCarouselEditor({ config, onChange, editorState }) {
  const [editingItemIndex, setEditingItemIndex] = useState(null);
  const [openPanels, setOpenPanels] = useState({ config: true, items: true });
  ...
  // ─── Drill-down: editing a specific item ──────────────────────────────────
  if (editingItemIndex !== null) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <DrillHeader onBack={() => { setEditingItemIndex(null); setEditingIcon(false); }} title={item.label || `Item ${editingItemIndex + 1}`} subtitle={`Item ${editingItemIndex + 1} de ${items.length}`} />
          ...
          <div>
            ...
            <EditorCard>
              ...
              <div style={{
                ...
                <div style={{
                  width: hasCustomImage ? '6.25rem' : '4rem', height: hasCustomImage ? '6.25rem' : '4rem',
                  borderRadius: hasCustomImage ? imageRadiusPreview : '0.75rem',
                  background: hasCustomImage ? 'transparent' : 'rgba(255,255,255,0.06)',
                  border: '0.0625rem solid var(--border-subtle)',
                  ...
                  {hasCustomImage ? (
                    <img
                      ...
                      alt=""
                      style={{ width: '100%', height: '100%', objectFit: itemImageFit, display: 'block' }}
                    />
                  ) : IconComp ? (
                    <IconComp size={28} color={item.iconColor || '#60a5fa'} />
                  ) : (
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.875rem', color: 'white', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {hasCustomImage ? 'Imagen personalizada' : (iconData?.name || 'Sin ícono')}
                  </div>
                  <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.6875rem', color: 'var(--text-dim)', marginTop: '0.125rem' }}>
                    {hasCustomImage ? 'PNG, SVG o JPG' : hasSystemIcon ? 'Ícono del sistema' : 'Ninguno seleccionado'}
                  </div>
                </div>
              </div>
              ...
              {editingIcon && (
                <div style={{ marginTop: '0.25rem' }}>
                  <IconSelector
                    value={item.icon}
                    onChange={newIcon => {
                      updateItem(editingItemIndex, { icon: newIcon, customImage: '' });
                      setEditingIcon(false);
                    }}
                  />
                </div>
          ...
          <div>
            ...
            <EditorCard>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <ToggleSwitch
                  checked={showLabel}
                  onChange={v => updateItem(editingItemIndex, { showLabel: v })}
                />
              </div>

              {showLabel && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                  <label style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.75rem', color: 'var(--text-dim)', fontWeight: 500 }}>
                    Contenido del texto
                  </label>
                  <TextInput
                    value={item.label || ''}
                    onChange={v => updateItem(editingItemIndex, { label: v })}
            ...
            <EditorCard>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                ...
                <Selector
                  ...
                  displayMap={{
                    none: { name: 'Sin enlace' },
                    external: { name: 'URL externa' },
                    page: { name: 'Página interna' }
                  }}
                  onChange={v => updateItem(editingItemIndex, { target: v, url: '', pageId: '' })}
                />
              </div>
              ...
              {item.target === 'external' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                  ...
                  <div style={{
                    ...
                    <input
                      value={item.url || ''}
                      onChange={e => updateItem(editingItemIndex, { url: e.target.value })}
                      style={{ ...inputStyle, border: 'none', borderRadius: 0, background: 'transparent', outline: 'none', flex: 1, padding: '0.5rem 0.625rem', fontSize: '0.8125rem' }}
                      placeholder="https://..."
                    />
                  </div>
                </div>
              ...
              {item.target === 'page' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                  ...
                  {pages.length > 0 ? (
                    <Selector
                      value={item.pageId || ''}
                      options={['', ...pages.map(p => p.id)]}
                      displayMap={{
                        '': { name: '— Seleccionar página —' },
                        ...pages.reduce((acc, p) => ({ ...acc, [p.id]: { name: p.label || p.name || p.id } }), {})
                      }}
                      onChange={v => updateItem(editingItemIndex, { pageId: v })}
                    />
                  ) : (
                    <EmptyText>
                      No hay páginas creadas en el proyecto aún.
                    </EmptyText>
                  )}
                </div>
              )}
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {showTitle && (
            <ControlGroup title="Título">
              <TextInput value={config.title || ''} onChange={v => updateConfig('title', v)} placeholder="Título del carrusel" />
            </ControlGroup>
          )}

          {/* Divider */}
          <div style={{ borderTop: '0.0625rem solid var(--border-subtle)', marginTop: '0.25rem', paddingTop: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            {/* Icon color */}
            <ControlGroup title="Color de íconos">
              <CustomColorPicker label="" value={config.iconColor || '#003DA6'} onChange={v => updateConfig('iconColor', v)} />
            </ControlGroup>

            {/* Text color */}
            <ControlGroup title="Color del texto">
              <CustomColorPicker label="" value={config.textColor || '#000000'} onChange={v => updateConfig('textColor', v)} />
            </ControlGroup>
          </div>
          ...
          <div style={{ borderTop: '0.0625rem solid var(--border-subtle)', paddingTop: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <ControlGroup title="Color de fondo">
              <CustomColorPicker label="" value={config.bgColor || '#ffffff'} onChange={v => updateConfig('bgColor', v)} />
            </ControlGroup>
      ...
      <Accordion title={`Elementos (${items.length})`} isOpen={openPanels.items} onToggle={() => togglePanel('items')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <ListEditor
            items={items}
            onItemsChange={v => updateConfig('items', v)}
            renderItemPreview={(item, index) => {
              const iconData = item.icon ? GOV_ICONS[item.icon] : null;
              const IconComp = iconData ? iconData.icon : null;
              return (
                <div
                  style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', overflow: 'hidden' }}
                  onClick={() => { setEditingItemIndex(index); setEditingIcon(false); }}
                >
                  <div style={{
                    width: '1.5rem', height: '1.5rem', flexShrink: 0,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    overflow: 'hidden', borderRadius: '0.25rem',
                    background: 'rgba(255,255,255,0.06)',
                  }}>
                    {item.customImage ? (
                      <img src={item.customImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} />
                    ) : IconComp ? (
                      <IconComp size={13} color={config.iconColor || '#60a5fa'} />
                    ) : (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {item.target && item.target !== 'none' && (
                      <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.625rem', color: 'var(--text-dim)', marginTop: '0.0625rem' }}>
                        {item.target === 'external' ? '↗ URL externa' : '→ Página interna'}
                      </div>
                    )}
                  </div>
                </div>
